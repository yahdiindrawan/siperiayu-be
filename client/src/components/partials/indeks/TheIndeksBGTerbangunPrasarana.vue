    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          <tbody
            v-if="indeksBGTerbangunPrasaranaData"
            class="divide-y divide-gray-200 bg-white"
          >
            <tr
              v-for="(data, index) in indeksBGTerbangunPrasaranaData"
              :key="data._id"
            >
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{
                          !indeksBGTerbangunPrasarana._id
                            ? "Tambah Data"
                            : "Edit Data"
                        }}
                      </DialogTitle>
                      ...
                      <div class="mt-6 space-y-3">
                        <div>
                          ...
                          <div class="mt-2">
                            <input
                              v-model="indeksBGTerbangunPrasarana.category"
                              id="kategori"
                              name="kategori"
                              type="text"
                        ...
                        <div>
                          ...
                          <div class="mt-2">
                            <input
                              v-model="indeksBGTerbangunPrasarana.indeks"
                              id="indeks"
                              name="indeks"
                              type="number"
                              min="0"
                  ...
                  >
                    ...
                    >
                      {{ !indeksBGTerbangunPrasarana._id ? "Save" : "Update" }}
                    </button>
                    <button
                      type="button"
...
const indeksBGTerbangunPrasarana = reactive({
  _id: null,
  category: null,
  indeks: null,
});
...
const indeksBGTerbangunPrasaranaData = ref(null);
...
const allIndeksBGTerbangunPrasarana = async () => {
  try {
    ...
    indeksBGTerbangunPrasaranaData.value = data.data;
  } catch (error) {
    console.log(error);
  }
};
...
const clearInput = () => {
  isModal.value = false;
  indeksBGTerbangunPrasarana._id = null;
  indeksBGTerbangunPrasarana.category = null;
  indeksBGTerbangunPrasarana.indeks = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  indeksBGTerbangunPrasarana._id = data._id;
  indeksBGTerbangunPrasarana.category = data.category;
  indeksBGTerbangunPrasarana.indeks = data.indeks;
};

const handleDelete = async (_id) => {
  try {
    const tempIndeksBGTerbangunPrasarana = await customFetch.delete(
      "/indeks/indeks-bg-terbangun-prasarana/" + _id
    );
    toast.message = tempIndeksBGTerbangunPrasarana.data.message;
    if (tempIndeksBGTerbangunPrasarana) {
...
const handleSubmit = async () => {
  try {
    let tempIndeksBGTerbangunPrasarana;
    if (!indeksBGTerbangunPrasarana._id) {
      tempIndeksBGTerbangunPrasarana = await customFetch.post(
        "/indeks/indeks-bg-terbangun-prasarana",
        {
          category: indeksBGTerbangunPrasarana.category,
          indeks: indeksBGTerbangunPrasarana.indeks,
        }
      );

      toast.message = tempIndeksBGTerbangunPrasarana.data.message;
    } else {
      tempIndeksBGTerbangunPrasarana = await customFetch.put(
        "/indeks/indeks-bg-terbangun-prasarana/" +
          indeksBGTerbangunPrasarana._id,
        {
          category: indeksBGTerbangunPrasarana.category,
