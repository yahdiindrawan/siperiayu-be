    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          <tbody
            v-if="indeksBGTerbangunData"
            class="divide-y divide-gray-200 bg-white"
          >
            <tr v-for="(data, index) in indeksBGTerbangunData" :key="data._id">
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{
                          !indeksBGTerbangun._id ? "Tambah Data" : "Edit Data"
                        }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
                      ...
                      <div class="mt-6 space-y-3 mx-4">
                        <div>
                          ...
                          <div class="mt-2">
                            <input
                              v-model="indeksBGTerbangun.category"
                              id="kategori"
                              name="kategori"
                              type="text"
                        ...
                        <div>
                          ...
                          <div class="mt-2">
                            <input
                              v-model="indeksBGTerbangun.indeks"
                              id="indeks"
                              name="indeks"
                              type="number"
                              min="0"
                  ...
                  >
                    ...
                    >
                      {{ !indeksBGTerbangun._id ? "Save" : "Update" }}
                    </button>
                    <button
                      type="button"
...
const indeksBGTerbangun = reactive({
  _id: null,
  category: null,
  indeks: null,
});
...
const clearInput = () => {
  isModal.value = false;
  indeksBGTerbangun._id = null;
  indeksBGTerbangun.category = null;
  indeksBGTerbangun.indeks = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  indeksBGTerbangun._id = data._id;
  indeksBGTerbangun.category = data.category;
  indeksBGTerbangun.indeks = data.indeks;
};

const handleDelete = async (_id) => {
  try {
    const tempIndeksBGTerbangun = await customFetch.delete(
      "/indeks/indeks-bg-terbangun/" + _id
    );
    toast.message = tempIndeksBGTerbangun.data.message;
    if (tempIndeksBGTerbangun) {
...
const handleSubmit = async () => {
  try {
    let tempIndeksBGTerbangun;
    if (!indeksBGTerbangun._id) {
      tempIndeksBGTerbangun = await customFetch.post(
        "/indeks/indeks-bg-terbangun",
        {
          category: indeksBGTerbangun.category,
          indeks: indeksBGTerbangun.indeks,
        }
      );

      toast.message = tempIndeksBGTerbangun.data.message;
    } else {
      tempIndeksBGTerbangun = await customFetch.put(
        "/indeks/indeks-bg-terbangun/" + indeksBGTerbangun._id,
        {
          category: indeksBGTerbangun.category,
          indeks: indeksBGTerbangun.indeks,
        }
