    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          >
            <tr v-for="(data, index) in pengaturanData" :key="data._id">
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{ !pengaturan._id ? "Tambah Data" : "Edit Data" }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
...
const handleEdit = (data) => {
  isModal.value = true;
  pengaturan._id = data._id;
  pengaturan.key = data.key;
  pengaturan.value = data.value;
};

const handleDelete = async (_id) => {
  try {
    const tempPengaturan = await customFetch.delete(
      "/settings/pengaturan/" + _id,
    );
    toast.message = tempPengaturan.data.message;
    if (tempPengaturan) {
      allPengaturan();
...
const handleSubmit = async () => {
  try {
    let tempPengaturan;
    if (!pengaturan._id) {
      tempPengaturan = await customFetch.post("/settings/pengaturan", {
        key: pengaturan.key,
      ...
      tempPengaturan = await customFetch.put(
        "/settings/pengaturan/" + pengaturan._id,
        {
          key: pengaturan.key,
          value: pengaturan.value,
        },
