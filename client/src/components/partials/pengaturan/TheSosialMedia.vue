    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          <tbody
            v-if="sosialMediaData"
            class="divide-y divide-gray-200 bg-white"
          >
            <tr v-for="(data, index) in sosialMediaData" :key="data._id">
              ...
              >
                ...
                <dl class="font-normal lg:hidden">
                  <dt class="sr-only sm:hidden">url</dt>
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{ !sosialMedia._id ? "Tambah Data" : "Edit Data" }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
...
const clearInput = () => {
  isModal.value = false;
  sosialMedia._id = null;
  sosialMedia.name = null;
  sosialMedia.url = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  sosialMedia._id = data._id;
  sosialMedia.name = data.name;
  sosialMedia.url = data.url;
};

const handleDelete = async (_id) => {
  try {
    const tempSosialMedia = await customFetch.delete(
      "/settings/sosial-media/" + _id,
    );
    toast.message = tempSosialMedia.data.message;
    if (tempSosialMedia) {
      allSosialMedia();
...
const handleSubmit = async () => {
  try {
    let tempSosialMedia;
    if (!sosialMedia._id) {
      tempSosialMedia = await customFetch.post("/settings/sosial-media", {
      ...
      tempSosialMedia = await customFetch.put(
        "/settings/sosial-media/" + sosialMedia._id,
        {
          name: sosialMedia.name,
          url: sosialMedia.url,
        },
