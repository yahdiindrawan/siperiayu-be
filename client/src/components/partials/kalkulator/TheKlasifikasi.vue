    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          <tbody
            v-if="klasifikasiData"
            class="divide-y divide-gray-200 bg-white"
          >
            <tr v-for="(data, index) in klasifikasiData" :key="data._id">
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{ !klasifikasi._id ? "Tambah Data" : "Edit Data" }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
                      ...
                      <div class="mt-6 space-y-3 mx-4">
                        <div>
                          ...
                          <div class="mt-2">
                            <input
                              v-model="klasifikasi.classification"
                              id="klasifikasi"
                              name="klasifikasi"
                              type="text"
...
const clearInput = () => {
  isModal.value = false;
  klasifikasi._id = null;
  klasifikasi.classification = null;
  klasifikasi.weight = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  klasifikasi._id = data._id;
  klasifikasi.classification = data.classification;
  klasifikasi.weight = data.weight;
};

const handleDelete = async (_id) => {
  try {
    const tempKlasifikasi = await customFetch.delete(
      "/kalkulator/klasifikasi/" + _id
    );
    toast.message = tempKlasifikasi.data.message;
    if (tempKlasifikasi) {
      allKlasifikasi();
...
const handleSubmit = async () => {
  try {
    let tempKlasifikasi;
    if (!klasifikasi._id) {
      tempKlasifikasi = await customFetch.post("/kalkulator/klasifikasi", {
        classification: klasifikasi.classification,
        weight: klasifikasi.weight,
      });

      toast.message = tempKlasifikasi.data.message;
    } else {
      tempKlasifikasi = await customFetch.put(
        "/kalkulator/klasifikasi/" + klasifikasi._id,
        {
          classification: klasifikasi.classification,
          weight: klasifikasi.weight,
        }
