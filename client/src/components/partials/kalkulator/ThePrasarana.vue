                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{ !prasarana._id ? "Tambah Data" : "Edit Data" }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
...
const prasarana = reactive({
  _id: null,
  category: null,
  building: null,
  satuan: null,
  hspbg: null,
  satuan_prasarana: null,
  charge: null,
  maksimum_per_unit: null,
});
...
const prasaranaData = ref(null);

const allPrasarana = async () => {
  try {
    const { data } = await customFetch.get("/kalkulator/prasarana");
    prasaranaData.value = data.data;
  } catch (error) {
    console.log(error);
  }
};
...
const clearInput = () => {
  isModal.value = false;
  prasarana._id = null;
  prasarana.category = null;
  prasarana.building = null;
  prasarana.satuan = null;
  prasarana.hspbg = null;
  prasarana.satuan_prasarana = null;
  prasarana.charge = null;
  prasarana.maksimum_per_unit = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  prasarana._id = data._id;
  prasarana.category = data.category;
  prasarana.building = data.building;
  prasarana.satuan = data.satuan;
  prasarana.hspbg = data.hspbg;
  prasarana.satuan_prasarana = data.satuan_prasarana;
  prasarana.charge = data.charge;
  prasarana.maksimum_per_unit = data.maksimum_per_unit;
};

const handleDelete = async (_id) => {
  try {
    const tempPrasarana = await customFetch.delete(
      "/kalkulator/prasarana/" + _id
    );
    toast.message = tempPrasarana.data.message;
    if (tempPrasarana) {
      allPrasarana();
...
const handleSubmit = async () => {
  try {
    let tempPrasarana;
    if (!prasarana._id) {
      tempPrasarana = await customFetch.post("/kalkulator/prasarana", {
        category: prasarana.category,
        building: prasarana.building,
        satuan: prasarana.satuan,
        hspbg: prasarana.hspbg,
        satuan_prasarana: prasarana.satuan_prasarana,
        charge: prasarana.charge,
        maksimum_per_unit: prasarana.maksimum_per_unit,
      });

      toast.message = tempPrasarana.data.message;
    } else {
      tempPrasarana = await customFetch.put(
        "/kalkulator/prasarana/" + prasarana._id,
        {
          category: prasarana.category,
          building: prasarana.building,
          satuan: prasarana.satuan,
