    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          <tbody
            v-if="kompleksitasData"
            class="divide-y divide-gray-200 bg-white"
          >
            <tr v-for="(data, index) in kompleksitasData" :key="data._id">
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{ !kompleksitas._id ? "Tambah Data" : "Edit Data" }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
...
const clearInput = () => {
  isModal.value = false;
  kompleksitas._id = null;
  kompleksitas.category = null;
  kompleksitas.indeks = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  kompleksitas._id = data._id;
  kompleksitas.category = data.category;
  kompleksitas.indeks = data.indeks;
};

const handleDelete = async (_id) => {
  try {
    const tempKompleksitas = await customFetch.delete(
      "/data-master/kompleksitas/" + _id
    );
    toast.message = tempKompleksitas.data.message;
    if (tempKompleksitas) {
      allKompleksitas();
...
const handleSubmit = async () => {
  try {
    let tempKompleksitas;
    if (!kompleksitas._id) {
      tempKompleksitas = await customFetch.post("/data-master/kompleksitas", {
        category: kompleksitas.category,
        indeks: kompleksitas.indeks,
      });

      toast.message = tempKompleksitas.data.message;
    } else {
      tempKompleksitas = await customFetch.put(
        "/data-master/kompleksitas/" + kompleksitas._id,
        {
          category: kompleksitas.category,
          indeks: kompleksitas.indeks,
        }
