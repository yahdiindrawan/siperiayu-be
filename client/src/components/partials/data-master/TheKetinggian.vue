    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          >
            <tr v-for="(data, index) in ketinggianData" :key="data._id">
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{ !ketinggian._id ? "Tambah Data" : "Edit Data" }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
...
const clearInput = () => {
  isModal.value = false;
  ketinggian._id = null;
  ketinggian.category = null;
  ketinggian.indeks = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  ketinggian._id = data._id;
  ketinggian.category = data.category;
  ketinggian.indeks = data.indeks;
};

const handleDelete = async (_id) => {
  try {
    const tempKetinggian = await customFetch.delete(
      "/data-master/ketinggian/" + _id
    );
    toast.message = tempKetinggian.data.message;
    if (tempKetinggian) {
      allKetinggian();
...
const handleSubmit = async () => {
  try {
    let tempKetinggian;
    if (!ketinggian._id) {
      tempKetinggian = await customFetch.post("/data-master/ketinggian", {
        category: ketinggian.category,
        indeks: ketinggian.indeks,
      });

      toast.message = tempKetinggian.data.message;
    } else {
      tempKetinggian = await customFetch.put(
        "/data-master/ketinggian/" + ketinggian._id,
        {
          category: ketinggian.category,
          indeks: ketinggian.indeks,
        }
