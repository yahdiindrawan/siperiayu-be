    ...
    <div class="mx-2 mt-8 sm:-mx-0">
      ...
      >
        <table class="min-w-full divide-y divide-gray-300">
          ...
          >
            <tr v-for="(data, index) in permanensiData" :key="data._id">
                ...
                <form @submit.prevent="handleSubmit" class="my-4">
                  <div>
                    <div class="mt-5">
                      ...
                      >
                        {{ !permanensi._id ? "Tambah Data" : "Edit Data" }}
                      </DialogTitle>
                      <CAlert
                        v-if="errorAlert"
...
const clearInput = () => {
  isModal.value = false;
  permanensi._id = null;
  permanensi.category = null;
  permanensi.indeks = null;
...
const handleEdit = (data) => {
  isModal.value = true;
  permanensi._id = data._id;
  permanensi.category = data.category;
  permanensi.indeks = data.indeks;
};

const handleDelete = async (_id) => {
  try {
    const tempPermanensi = await customFetch.delete(
      "/data-master/permanensi/" + _id
    );
    toast.message = tempPermanensi.data.message;
    if (tempPermanensi) {
      allPermanensi();
...
const handleSubmit = async () => {
  try {
    let tempPermanensi;
    if (!permanensi._id) {
      tempPermanensi = await customFetch.post("/data-master/permanensi", {
        category: permanensi.category,
        indeks: permanensi.indeks,
      });

      toast.message = tempPermanensi.data.message;
    } else {
      tempPermanensi = await customFetch.put(
        "/data-master/permanensi/" + permanensi._id,
        {
          category: permanensi.category,
          indeks: permanensi.indeks,
        }
